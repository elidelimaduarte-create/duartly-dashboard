...
// ============================================================
// DESIGN TOKENS — Identidade Duartly
// ============================================================
const tokens = {
  bg:        '#0f0f1a',
  bgCard:    '#1a1a2e',
  bgCardAlt: '#16213e',
  accent:    '#4ade80',   // verde lhama
  accentSoft:'#22c55e',
  danger:    '#f87171',
  warning:   '#fbbf24',
  text:      '#f0fdf4',
...
function Stat({ label, value, sub, color = tokens.text }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <span style={{ fontSize: 11, color: tokens.textMuted, textTransform: 'uppercase', letterSpacing: 1 }}>
        {label}
      </span>
      <span style={{ fontSize: 28, fontWeight: 700, color, fontFamily: 'Space Mono, monospace' }}>
        {value}
      </span>
  ...
  return (
    <div style={{
      ...
      <p style={{ color: tokens.textSub, textAlign: 'center', maxWidth: 320, lineHeight: 1.6 }}>
        Este link de dashboard expirou ou é inválido.<br />
...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{
      ...
      {payload.map((p, i) => (
        <p key={i} style={{ color: p.color || tokens.accent, fontWeight: 600 }}>
          {formatBRL(p.value)}
        </p>
      ))}
    </div>
...
export default function App() {
  ...
  const [filtro, setFiltro] = useState('mes'); // 'hoje' | 'semana' | 'mes'
  ...
  async function validarECarregar(token) {
    try {
      ...
      await carregarDados(usuarioId, tokenData.usuarios);
  ...
  async function carregarDados(usuarioId, usuario) {
    ...
    const { data: metas } = await supabase
      ...
      .select('*, categorias(nome, emoji)')
      .eq('usuario_id', usuarioId)
      .eq('mes', getMesAtual().mes)
      .eq('ano', getMesAtual().ano);
    ...
    setDados({ transacoes, parcelas, metas, historico, usuario, nomeMes });
  }

  if (loading) return <TelaLoading />;
  if (!tokenValido || !dados) return <TelaInvalida />;

  const { transacoes, parcelas, metas, historico, usuario, nomeMes } = dados;
  ...
  // Por categoria (para gráfico pizza)
  const porCategoria = {};
  gastos.forEach(t => {
  ...
  const dadosPizza = Object.values(porCategoria)
  ...
  historico?.forEach(t => {
    ...
    if (!dadosHistorico[chave]) dadosHistorico[chave] = { mes: chave, gastos: 0, receitas: 0 };
    if (t.tipo === 'gasto') dadosHistorico[chave].gastos += parseFloat(t.valor);
    else dadosHistorico[chave].receitas += parseFloat(t.valor);
  });
  const dadosArea = Object.values(dadosHistorico).slice(-6);

  // Insights dos agentes
  const maiorCategoria = dadosPizza[0];
  const totalParcelas = parcelas?.reduce((a, p) => a + parseFloat(p.valor), 0) || 0;
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 16px 60px' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: tokens.textSub }}>
                Olá, {usuario?.nome || 'usuário'} · {nomeMes}
              </p>
            </div>
          </div>
        ...
        <div style={{
          ...
          <Card>
            <Stat
              ...
              color={saldo >= 0 ? tokens.accent : tokens.danger}
              sub={`${nomeMes}`}
            />
          </Card>
          <Card>
          ...
          <Card>
            <Stat
              ...
              color={tokens.accent}
              sub={`${receitas.length} entradas`}
            />
          </Card>
          <Card>
            <Stat
              label="Parcelas ativas"
              value={formatBRL(totalParcelas)}
              color={tokens.warning}
              sub={`${parcelas?.length || 0} pendentes`}
            />
          </Card>
        </div>
        ...
        {dadosArea.length > 0 && (
          <Card style={{ marginBottom: 24 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h2 style={{ fontSize: 15, fontWeight: 600 }}>Histórico 6 meses</h2>
              <Badge color={tokens.accentSoft}>Evolução</Badge>
            </div>
              ...
              <ResponsiveContainer width="100%" height={180}>
                <PieChart>
                  <Pie data={dadosPizza} dataKey="total" nameKey="nome" cx="50%" cy="50%" outerRadius={70} strokeWidth={0}>
                    {dadosPizza.map((_, i) => (
                      <Cell key={i} fill={CATEGORIA_CORES[i % CATEGORIA_CORES.length]} />
                    ))}
                  </Pie>
              ...
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={dadosPizza.slice(0, 5)} layout="vertical">
                  ...
                  <Bar dataKey="total" radius={[0, 6, 6, 0]}>
                    {dadosPizza.slice(0, 5).map((_, i) => (
                      <Cell key={i} fill={CATEGORIA_CORES[i]} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
        ...
        <Card style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <AgentCard
              ...
              mensagem={
                maiorCategoria
                  ? `Seu maior gasto este mês foi em ${maiorCategoria.emoji} ${maiorCategoria.nome}: ${formatBRL(maiorCategoria.total)}. Fique de olho!`
                  : 'Nenhum gasto registrado ainda este mês. Me manda um gasto no bot!'
              }
            />
            <AgentCard
              nome="Luna"
              ...
              mensagem={
                totalGastos > 0
                  ? `Média diária de gastos: ${formatBRL(totalGastos / new Date().getDate())}. ${totalGastos > totalReceitas ? 'Atenção: gastos acima das receitas!' : 'Bom ritmo financeiro!'}`
                  : 'Registre seus gastos para eu analisar as tendências quinzenais.'
              }
            />
            <AgentCard
              nome="Inti"
              ...
              mensagem={
                totalGastos > 0
                  ? `No ritmo atual, você deve gastar ${formatBRL(totalGastos / new Date().getDate() * 30)} este mês. Saldo projetado: ${formatBRL(totalReceitas - (totalGastos / new Date().getDate() * 30))}.`
                  : 'Aguardando dados suficientes para projeção mensal.'
              }
            />
          </div>
        </Card>
        ...
        <Card style={{ marginBottom: 24 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <h2 style={{ fontSize: 15, fontWeight: 600 }}>Últimas transações</h2>
            <span style={{ fontSize: 12, color: tokens.textSub }}>{transacoes?.length || 0} este mês</span>
          </div>
          {transacoes?.length === 0 && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {transacoes?.slice(0, 10).map((t, i) => (
              <div key={i} style={{
                display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                padding: '10px 0',
                borderBottom: i < 9 ? `1px solid ${tokens.border}` : 'none',
              }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 11, color: tokens.textSub }}>
                      {t.categorias?.nome || 'Outros'} · {formatData(t.data_transacao)}
                    </p>
                  </div>
                </div>
                <span style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {parcelas.slice(0, 8).map((p, i) => (
                <div key={i} style={{
                  display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                  padding: '10px 0',
                  borderBottom: i < parcelas.length - 1 ? `1px solid ${tokens.border}` : 'none',
                }}>
                  <div>
                    <p style={{ fontSize: 13, fontWeight: 500 }}>{p.descricao}</p>
                    <p style={{ fontSize: 11, color: tokens.textSub }}>
                      Vence em {formatData(p.data_transacao)}
                    </p>
                  </div>
                  <span style={{
                    fontSize: 14, fontWeight: 700,
